...
static volatile bool DRV_SpeedOn = FALSE, DRV_PosOn = FALSE;
...
void DRV_Pos_EnableDisable(bool enable) {
	DRV_PosOn = enable;
}
...
static portTASK_FUNCTION(DriveTask, pvParameters) {
	...
	for (;;) {
		...
		prevOn = DRV_SpeedOn;
		if (prevPosOn && !DRV_PosOn) { /* turned off */
			MOT_SetSpeedPercent(MOT_GetMotorHandle(MOT_MOTOR_LEFT), 0);
			MOT_SetSpeedPercent(MOT_GetMotorHandle(MOT_MOTOR_RIGHT), 0);
			PID_Pos_Start(); /* reset values */
		} else if (DRV_PosOn) {
			PID_Pos(TACHO_GetPos(TRUE), DRV_PosLeft, DRV_SpeedLeft,
					DRV_SpeedRight, TRUE); /* left */
			PID_Pos(TACHO_GetPos(FALSE), DRV_PosRight, DRV_SpeedLeft,
					DRV_SpeedRight, FALSE); /* right */
		}
		prevPosOn = DRV_PosOn;
...
static void DRV_PrintStatus(const CLS1_StdIOType *io) {
	...
	CLS1_SendStatusStr((unsigned char*) "  pos",
			DRV_PosOn ? (unsigned char*) "on\r\n" : (unsigned char*) "off\r\n",
			io->stdOut);
	buf[0] = '\0';
...
static void DRV_PrintHelp(const CLS1_StdIOType *io) {
	...
	CLS1_SendHelpStr((unsigned char*) "  pos (on|off)",
			(unsigned char*) "Turns pos pid on or ff\r\n", io->stdOut);
	CLS1_SendHelpStr((unsigned char*) "  pos (L|R) <value>",
			(unsigned char*) "Sets pos value\r\n", io->stdOut);
}
...
uint8_t DRV_ParseCommand(const unsigned char *cmd, bool *handled,
	...
	} else if (UTIL1_strncmp((char* )cmd, (char* )"drive pos R",
			sizeof("drive pos R") - 1)
			== 0) {
	...
	} else if (UTIL1_strncmp((char* )cmd, (char* )"drive pos",
			sizeof("drive pos") - 1)
			== 0) {
