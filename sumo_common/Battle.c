...
uint8_t BATTLE_ParseCommand(const unsigned char *cmd, bool *handled,
	...
	if (UTIL1_strcmp((char*)cmd, (char*)CLS1_CMD_HELP) == 0
		...
		*handled = TRUE;
			...
			|| UTIL1_strcmp((char*)cmd, (char*)"battle status") == 0) {
		BATTLE_PrintStatus(io);
		*handled = TRUE;
	} else if (UTIL1_strcmp((char*)cmd, (char*)CLS1_CMD_STATUS) == 0
			|| UTIL1_strcmp((char*)cmd, (char*)"battle on") == 0) {
		battleState = BATTLE_STATE_FIND;
	} else if (UTIL1_strcmp((char*)cmd, (char*)CLS1_CMD_STATUS) == 0
			|| UTIL1_strcmp((char*)cmd, (char*)"battle start") == 0) {
		battleState = BATTLE_STATE_WAIT;
	} else if (UTIL1_strcmp((char*)cmd, (char*)CLS1_CMD_STATUS) == 0
			|| UTIL1_strcmp((char*)cmd, (char*)"battle off") == 0) {
		battleState = BATTLE_STATE_NONE;
	} else if (UTIL1_strcmp((char*)cmd, (char*)CLS1_CMD_STATUS) == 0
			|| UTIL1_strcmp((char*)cmd, (char*)"battle remote") == 0) {
